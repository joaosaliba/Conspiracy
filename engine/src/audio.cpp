/**class to object of audio, where you can find some variables, files, others objects
...
void Audio::play(const int timesToPlay) {
    if(audio_music != NULL) {
        if(Mix_PlayMusic(audio_music, timesToPlay) == -1) {
            ERROR("Audio could not be played");
        }
    }else {
        if(Mix_PlayChannel(-1, audio_effect, timesToPlay) == -1) {
            ERROR("Audio could not be played");
        }
    }
}
